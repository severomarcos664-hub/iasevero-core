...
try {
  ...
  const auditAssessmentEvaluatedAt =
    '2026-07-22T00:00:30.000Z'

  const auditAssessment = {
    assessmentVersion: 1 as const,
    assessmentId: 'assessment-v283-16-end-to-end',
    tenantId,
    userId,
    memoryId,
    evaluatedAt: auditAssessmentEvaluatedAt,
    utilityScore: 100,
    recommendation: 'retain' as const,
    signals: [],
    evidence: [],
    reasoning: [
      'Assessment persistido para comprovar a cadeia end-to-end v283.16.',
    ],
    mutationApplied: false as const,
  }

  assert.equal(auditAssessment.tenantId, tenantId)
  assert.equal(auditAssessment.userId, userId)
  assert.equal(auditAssessment.memoryId, memoryId)
  assert.equal(auditAssessment.mutationApplied, false)

  repository.appendMemoryUtilityAssessment({
    assessment: auditAssessment,
    createdAt: auditAssessmentEvaluatedAt,
  })

  const persistedAuditAssessmentHistory =
  ...
  assert.equal(
    persistedAuditAssessmentHistory.length,
    1,
  )

  const persistedAuditAssessment =
    persistedAuditAssessmentHistory[0]

  assert.ok(persistedAuditAssessment)

  assert.equal(
    persistedAuditAssessment.assessmentId,
    auditAssessment.assessmentId,
  )

  assert.equal(
    persistedAuditAssessment.tenantId,
    tenantId,
  )

  assert.equal(
    persistedAuditAssessment.userId,
    userId,
  )

  assert.equal(
    persistedAuditAssessment.memoryId,
    memoryId,
  )

  assert.equal(
    persistedAuditAssessment.mutationApplied,
    false,
  )
  ...
  console.log(
    'Runtime governed memory end-to-end audit source workflow passed.',
  )
  ...
  const endToEndAudit =
    repository.readMemoryEndToEndAudit({
      tenantId,
      userId,
  ...
  assert.equal(endToEndAudit.workflowVersion, 1)
  assert.equal(endToEndAudit.tenantId, tenantId)
  assert.equal(endToEndAudit.userId, userId)

  assert.equal(
    endToEndAudit.requestId,
    authorization.requestId,
  )

  assert.equal(
    endToEndAudit.authorizationId,
    authorization.authorizationId,
  )

  assert.equal(
    endToEndAudit.executionId,
    executionId,
  )

  assert.equal(
    endToEndAudit.memoryId,
    authorization.memoryId,
  )

  assert.equal(
    endToEndAudit.decisionId,
    authorization.decisionId,
  )

  assert.equal(
    endToEndAudit.stagesPresent.assessment,
    true,
  )

  assert.equal(
    endToEndAudit.stagesPresent.review,
    true,
  )

  assert.equal(
    endToEndAudit.stagesPresent.authorization,
    true,
  )

  assert.equal(
    endToEndAudit.stagesPresent.execution,
    true,
  )

  assert.equal(endToEndAudit.chainComplete, true)
  assert.equal(endToEndAudit.integrityValid, true)
  assert.equal(endToEndAudit.violations.length, 0)
  assert.equal(endToEndAudit.executionApplied, false)
  assert.equal(endToEndAudit.mutationApplied, false)

  assert.ok(endToEndAudit.assessmentCount > 0)
  assert.ok(endToEndAudit.reviewEventCount > 0)
  assert.ok(endToEndAudit.authorizationEventCount > 0)
  assert.ok(endToEndAudit.executionEventCount > 0)

  assert.equal(
    endToEndAudit.totalEventCount,
    endToEndAudit.timeline.length,
  )

  for (
    let index = 1;
    index < endToEndAudit.timeline.length;
    index += 1
  ) {
    assert.ok(
      endToEndAudit.timeline[index - 1].createdAt <=
        endToEndAudit.timeline[index].createdAt,
    )
  }

  const stages =
    new Set(
      endToEndAudit.timeline.map(
        (entry) => entry.stage,
      ),
    )

  assert.equal(stages.has('assessment'), true)
  assert.equal(stages.has('review'), true)
  assert.equal(stages.has('authorization'), true)
  assert.equal(stages.has('execution'), true)

  let crossUserAuditBlocked = false
  ...
  try {
    ...
    crossUserAuditBlocked = true
  }

  assert.equal(crossUserAuditBlocked, true)

  let invalidExecutionScopeBlocked = false
  ...
  try {
    repository.readMemoryEndToEndAudit({
      ...
      requestId: authorization.requestId,
      authorizationId: authorization.authorizationId,
      executionId: 'missing-execution-v283-16',
    })
  } catch {
    invalidExecutionScopeBlocked = true
  }

  assert.equal(invalidExecutionScopeBlocked, true)

  console.log(
    'Runtime governed memory end-to-end audit proof passed.',
  )

  console.log({
    chainComplete:
      endToEndAudit.chainComplete,
    integrityValid:
      endToEndAudit.integrityValid,
    stagesPresent:
      endToEndAudit.stagesPresent,
    totalEventCount:
      endToEndAudit.totalEventCount,
    crossUserAuditBlocked,
    invalidExecutionScopeBlocked,
    executionApplied:
      endToEndAudit.executionApplied,
    mutationApplied:
      endToEndAudit.mutationApplied,
  })
